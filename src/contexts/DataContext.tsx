...
export function DataProvider({ children }: { children: ReactNode }) {
  ...
  const [eventSpeakers, setEventSpeakers] =
    useState<EventSpeaker[]>(initialEventSpeakers);
  ...
  const addEventSpeaker = useCallback(
    (eventSpeaker: Omit<EventSpeaker, "id">) => {
      setEventSpeakers((prev) => [
        ...prev,
        { ...eventSpeaker, id: generateId() },
      ]);
    },
    [],
  );
  ...
  const updateEventSpeaker = useCallback(
    (id: string, eventSpeaker: Partial<EventSpeaker>) => {
      setEventSpeakers((prev) =>
        prev.map((es) => (es.id === id ? { ...es, ...eventSpeaker } : es)),
      );
    },
    [],
  );
  ...
  const updateTeamMember = useCallback(
    (id: string, member: Partial<TeamMember>) => {
      setTeamMembers((prev) =>
